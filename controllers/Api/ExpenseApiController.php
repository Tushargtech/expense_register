...
class ExpenseApiController extends ApiBaseController
{
    ...
    public function viewAttachment(): void
    {
        ...
        $input = $this->input();
        $attachmentId = (int) ($input['attachment_id'] ?? $_GET['attachment_id'] ?? 0);
        $requestId = (int) ($input['request_id'] ?? $_GET['request_id'] ?? 0);

        if ($attachmentId <= 0 || $requestId <= 0) {
            $this->jsonError('Invalid attachment request. Parameters: attachment_id, request_id (both required)', 400);
            return;
        }
        ...
        try {
            ...
            if ($filePath === '') {
                $this->jsonError('Attachment file path not found.', 500);
                return;
            }
            ...
            if (!is_file($absolutePath) || !is_readable($absolutePath)) {
                $this->jsonError('Attachment file is not available on the server.', 500);
                return;
            }

            $fileContents = file_get_contents($absolutePath);
            if ($fileContents === false) {
                $this->jsonError('Failed to read attachment file.', 500);
                return;
            }
            ...
            error_log('ExpenseApiController::viewAttachment failed: ' . $error->getMessage());
            $this->jsonError('An error occurred while retrieving the attachment.', 500);
        }
    }
    ...
    {
        if ($this->method() !== 'POST') {
            $this->jsonError('Method not allowed. Use POST method.', 405);
            return;
        }
        ...
        $comment = trim((string) ($input['comment'] ?? $_POST['comment'] ?? ''));
        $reassignTo = (int) ($input['reassign_to'] ?? $_POST['reassign_to'] ?? 0);

        if ($requestId <= 0) {
            $this->jsonError('Request ID is required.', 400);
            return;
        }

        if ($action === '') {
            $this->jsonError('Action is required.', 400);
            return;
        }

        if (!in_array($action, ['approve', 'reject', 'reassign'], true)) {
            $this->jsonError('Invalid action. Allowed: approve, reject, reassign.', 400);
            return;
        }
        ...
        try {
            ...
            $actorUserId = (int) ($this->authenticatedUser()['user_id'] ?? 0);
            if ($actorUserId <= 0) {
                $this->jsonError('Unable to identify the current user.', 401);
                return;
            }
            ...
            error_log('ExpenseApiController::action failed: ' . $error->getMessage());
            $this->jsonError($error->getMessage(), 400);
        }
    }
}