				...
				<div class="filter-layout">
					<div class="filter-left">
						<div class="filter-grid">
							...
							<div class="filter-field">
								<input type="date" name="date_to" class="form-control" value="<?php echo htmlspecialchars($dateToValue, ENT_QUOTES, 'UTF-8'); ?>" placeholder="To">
							</div>
				...
				<table class="table user-list-table align-middle mb-0">
					...
					<tbody>
						<?php if (empty($requests)): ?>
							<tr>
								<td colspan="11" class="text-center py-4 text-muted">No expense requests found.</td>
							</tr>
						<?php else: ?>
							<?php foreach ($requests as $index => $row): ?>
								...
								$date = (string) ($row['request_submitted_at'] ?? '');
								$formattedDate = $date !== '' ? date('d M Y, h:i A', strtotime($date)) : '';
								?>
								<tr>
